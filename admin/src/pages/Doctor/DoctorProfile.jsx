...
const DoctorProfile = () => {
  ...
  const updateProfile = async () => {
    try {
      ...
      const {data} = await axios.post(backendUrl + '/api/caregiver/update-profile', updateData, {headers: {dToken}})
      ...
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          <div className='flex items-center gap-2 mt-1 text-gray-600'>
            ...
            <button className='py-0.5 px-2 border text-xs rounded-full hover:bg-primary hover:text-white'>{profileData.experience}</button>
          </div>
          ...
          <div className='flex gap-2 py-2'>
            <p>Address: </p> <p className='text-sm'>
